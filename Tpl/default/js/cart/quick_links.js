	...
	showQuickPop = function(type){
		...
		if(type !== prevPopType){
			...
			if(fn && fn.msgtype==1){
				...
				fn.content = "<div class='ibar_plugin_content' style='height:100%;padding-top:100%;padding-left:80px;'><img src='"+domainURL +"./Tpl/default/images/loading.gif' width='20'/>数据加载中...</div>";
				...
				jQuery.post(Think.U('Home/Cart/getCartInfo') ,{"axm":1},function(data) {
						...
						for(var goodsId in shop.shopgoods){
							...
							html.push(  "<li class='cart_item'>" +
										...
										"<div class='cart_item_desc'>" +
											"<a href='"+url+"' class='cart_item_name'>" );
							if(goods.isGroup==1){
								html.push(goods.goodsName+"<span style='color:#e6126a'>【团购】</span></a>");
							}else if(goods.isSeckill==1){
								html.push(goods.goodsName+"<span style='color:#e6126a'>【秒杀】</span></a>");
							}else{
								html.push(goods.goodsName+"</a>");
							}
							var shopPrice = 0;
							for(var p in goods){
								if(!isNaN(p)){
									if(goods[p].attrName){
										html.push(  "<div class='cart_item_price'>"+goods[p].attrName+"："+goods[p].attrVal+"</div>" );
									}
							...
							html.push(  "<div class='cart_item_price'><span class='cart_price'>￥"+goods.shopPrice+"</span></div>" +
											...
											"<span class='cart-colse' onclick=removeCartGoods(this,'"+goods.goodsId+"','"+goods.goodsAttrId+"','"+goods.isGroup+"','"+goods.isSeckill+"')></span></div>	" +
					...
					html.push('</ul></div><div class="cart_handler"><div class="cart_handler_header"><span class="cart_handler_left">共<span class="cart_price">'+goodsnum+'</span>件商品</span><span class="cart_handler_right">￥'+cart.totalMoney+'</span></div>');
					html.push('<div style="width:260px;"><a href="javascript:topay();" class="cart_go_btn" >去购物车结算</a></div></div></div>');
...
function removeCartGoods(obj,goodsId,goodsAttrId,isGroup,isSeckill){
	jQuery.post(Think.U('Home/Cart/delCartGoods') ,{goodsId:goodsId,goodsAttrId:goodsAttrId,isGroup:isGroup,isSeckill:isSeckill},function(data) {
