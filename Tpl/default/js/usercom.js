...
//设置或修改支付密码
function editPassPay(){
	   var payPwd = $('#payPwd').val();
	   var params = {};
	   if(payPwd==1){
		   params.oldPassPay = $('#oldPassPay').val();
		   params.newPassPay = $('#newPassPay').val();
		   params.reNewPassPay = $('#reNewPassPay').val();
		}else{
		   params.passPay = $('#passPay').val();
		   params.rePassPay = $('#rePassPay').val();	
		}
	   var ll = layer.load('数据处理中，请稍候...');
	   $.post(Think.U("Home/Users/editPassPay"),params,function(data,textStatus){
		   layer.close(ll);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('密码修改成功!', {icon: 1}, function(){
...
function toPay(id,payway){
	...
	jQuery.post(Think.U('Home/Orders/checkOrderPay') ,params,function(data) {
		var json = WST.toJson(data);
		if(json.status==1){
			location.href=Think.U('Home/Payments/toPay','orderIds='+params.orderIds+'&payway='+params.payway);
		}else if(json.status==-2){
...
function showRefund(id){
	layer.open({
	    ...
	    border: [0],
	    offset: ['20px',''],
	    content: [Think.U('Home/Orders/getRefundDetails','orderId='+id)],
	    area: ['500px', ($(window).height() - 200) +'px']
	});
}
function showIntegralOrders(id){
	layer.open({
	    type: 2,
	    title:"订单详情",
	    shade: [0.6, '#000'],
	    border: [0],
	    offset: ['20px',''],
	    content: [Think.U('Home/IntegralOrders/getOrderDetails','orderId='+id)],
	    area: ['1020px', ($(window).height() - 50) +'px']
	});
}
function showShare(id){
	layer.open({
	    ...
	    border: [0],
	    offset: ['20px',''],
	    content: [Think.U('Home/Orders/shareList','orderId='+id)],
	    area: ['1020px', ($(window).height() - 50) +'px']
	});
}
    //查看物流
    function shopOrderExpress(id){
        layer.open({
            type: 2,
            title:"物流跟踪",
            shade: [0.6, '#000'],
            border: [0],
            content: [Think.U('Home/Orders/shopOrderExpress','orderId='+id)],
            area: ['1020px', ($(window).height() - 50) +'px']
        });
    }
function orderCancel(id,type,shop,needPay,deliverMoney){
	if(type==1 || type==2){
		var w = layer.open({
		    type: 1,
		    title:"取消",
		    shade: [0.6, '#000'],
		    border: [0],
		    content: '取消原因<span style="color:red">*</span>：<select id="reason" style="width:180px;margin-left:3px;"><option valule="1">拍错了/订单信息错误</option><option valule="1">不想买了</option></select><br />退款金额<span style="color:red">*</span>：<input style="width:170px" type="text" disabled="true" value="'+needPay+'(含运费￥'+deliverMoney+'元)" ><br />取消说明：<textarea id="rejectionRemarks" rows="8" style="width:70%" maxLength="100"></textarea>',
		    area: ['500px', '310px'],
		    btn: ['提交', '关闭窗口'],
	        yes: function(index, layero){
	        	...
	        	var ll = layer.load('数据处理中，请稍候...');
			    $.post(Think.U('Home/Orders/orderCancel'),{orderId:id,type:1,reason:reason,rejectionRemarks:rejectionRemarks,needPay:needPay,shop:shop},function(data){
			    	layer.close(w);
			    	layer.close(ll);
			    	var json = WST.toJson(data);
					if(json.status>0){
		...
		layer.confirm('您确定要取消该订单吗？',{icon: 3, title:'系统提示'}, function(tips){
		    var ll = layer.load('数据处理中，请稍候...');
		    if(shop!=0){
		    	var url = "Home/Orders/orderCancel";
		    }else{
		    	var url = "Home/IntegralOrders/orderCancel";
		    }
...
//晒单
function addShareAppraises(shopId,goodsId,orderId){
	var shareTitle = $.trim($('#'+goodsId+'_title').val());
	if(shareTitle.length<3 || shareTitle.length>15){
		WST.msg('标题为3-15个字 !', {icon: 5});
		return;
	}
	var shareContent = $.trim($('#'+goodsId+'_content').val());
	if(shareContent.length<3 || shareContent.length>50){
		WST.msg('晒单内容为3-50个字 !', {icon: 5});
		return;
	}
		$.post(Think.U('Home/Share/addshare'),{shopId:shopId, goodsId:goodsId, orderId:orderId, shareTitle:shareTitle,shareContent:shareContent },function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status==1){
				$('#'+goodsId+'_appraise').slideUp();
				$('#'+goodsId+'_appraise').empty();
				$('#'+goodsId+'_status').html('晒单成功');
			}else{
				WST.msg(json.status, {icon: 3});
			}
		});
}
...
function addGoodsAppraises(shopId,goodsId,orderId){
	...
	var packScore = $('.'+goodsId+'_packScore > input[name="score"]').val();
	if(packScore==0){
		WST.msg('请选择商品包装满意度 !', {icon: 5});
		return;
	}
	
	var timeScore = $('.'+goodsId+'_timeScore > input[name="score"]').val();
	if(timeScore==0){
		WST.msg('请选择送货速度满意度 !', {icon: 5});
		return;
	}
	
	var serviceScore = $('.'+goodsId+'_serviceScore > input[name="score"]').val();
	if(serviceScore==0){
		WST.msg('请选择配送人员服务满意度 !', {icon: 5});
		return;
	}
	...
	var anonymity = $('#'+goodsId+'_anonymity').is(":checked")?0:1;

	//layer.confirm('您确定要提交该评价吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
		$.post(Think.U('Home/GoodsAppraises/addGoodsAppraises'),{shopId:shopId, goodsId:goodsId, orderId:orderId, goodsScore:goodsScore, packScore:packScore, timeScore:timeScore, serviceScore:serviceScore, content:content, anonymity:anonymity },function(data,textStatus){
			//layer.close(tips);
			layer.close(ll);
			var json = WST.toJson(data);
...
function orderConfirm(id,type,shop,needPay,deliverMoney){
	var needPay = arguments[3]?arguments[3]:0;
	var deliverMoney = arguments[4]?arguments[4]:0;
	if(type==1){
		...
		var w = layer.open({
		    ...
		    shade: [0.6, '#000'],
		    border: [0],
		    content: '拒收原因<span style="color:red">*</span>：<select id="reason" style="width:180px;margin-left:3px;"><option valule="1">拍错了/订单信息错误</option><option valule="1">不想买了</option></select><br />退款金额<span style="color:red">*</span>：<input style="width:170px" type="text" disabled="true" value="'+needPay+'(含运费￥'+deliverMoney+'元)" ><br />拒收说明：<textarea id="rejectionRemarks" rows="8" style="width:70%" maxLength="100"></textarea>',
		    area: ['500px', '310px'],
		    btn: ['拒收订单', '关闭窗口'],
	        yes: function(index, layero){
	        	var reason = $.trim($('#reason').val());
	        	var rejectionRemarks = $.trim($('#rejectionRemarks').val());
	        	var ll = layer.load('数据处理中，请稍候...');
			    $.post(Think.U('Home/Orders/orderConfirm'),{orderId:id,type:type,rejectionRemarks:rejectionRemarks,reason:reason,shop:shop,needPay:needPay},function(data){
			    	layer.close(w);
			    	layer.close(ll);
			    	var json = WST.toJson(data);
					if(json.status>0){
...
function queryAuctionOrderPager(statusMark,pcurr,id,skId){
    ...
    param.shopName = $.trim($("#shopName_"+statusMark).val());
    ...
    $.post(Think.U('Home/Orders/queryUserAuctionOrders'),param,function(data,textStatus){
        var json = WST.toJson(data);
        var html = new Array();
        ...
        if(json.root.length>0){
            for(var i=0;i<json.root.length;i++){
                ...
                html.push("<td width='100'><a href='javascript:;'  onclick=showOrder('"+order.orderId+"')>"+order.orderNo+"</a></td>");
                html.push("<td width='*'>"+order.goodsName+"</td>");
                html.push("<td width='*'>"+order.shopName+"</td>");
                html.push("<td width='100'>"+order.totalMoney+"</td>");
                if(order.orderStatus==6 || order.orderStatus==7){
                    html.push("<td width='100'>"+order.lastTime+"</td>");
                }else{
                ...
                if(order.orderStatus==6){
                    html.push(" | <a href="+Think.U('Home/Orders/checkAuctionOrderInfo/','goodsId='+order.goodsId+'&orderId='+order.orderId)+">提交订单</a>");
                }
                if(order.orderStatus==0 || order.orderStatus==1 || order.orderStatus==2){
                    html.push(" | <a href='javascript:orderCancel("+order.orderId+",0,"+order.goodsId+")'>取消订单</a>");
                }
                if(order.orderStatus==3){
                    html.push(" | <a href='javascript:orderConfirm("+order.orderId+",1)'>确认收货</a>");
                    html.push(" | <a href='javascript:orderConfirm("+order.orderId+",-1)'>拒收</a>");
                }
                if(order.orderStatus==4){
                    if(order.isAppraises==1){
                        html.push(" | 已评价");
                    }else{
                        html.push(" | <a href='javascript:appraiseOrder("+order.orderId+")'>评价</a>");
                    }

                    html.push(" | <a href='javascript:showShare("+order.orderId+")'>晒单</a>");
                }
                html.push("</td>");
                html.push("</tr>");
            }
        ...
        if(json.totalPage>1){
            laypage({
                ...
                jump: function(e, first){
                    if(!first){
                        queryAuctionOrderPager(statusMark,e.curr);
                    }
                }
            });
        }else{
