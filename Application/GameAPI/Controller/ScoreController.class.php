...
class ScoreController extends BaseController
{
    ...
    public function record()
    {
        ...
        $orderList = M('orders')->field($field)
            ->where($map)
            ->join('as o left join oto_score_record as m on o.orderNo=m.orderNo left join oto_order_goods as g on g.orderId=o.orderId')
            ->select();
    ...
    {
        $catid = I('catid');
        $this->catName = M('integral_cats')->where(array(
            'catId' => $catid
        ))->getField('catName');
        // 积分商品列表
        $smap['goodsStock'] = array(
            'gt',
            0
        );
        $smap['goodsFlag'] = 1;
    ...
    public function convertDetail()
    {
        ...
        $id = I('id');
        if (! $id) {
            $this->redirect(U('Index/index', array(
                'r' => 'score'
            ), '', '', 0));
            return;
        }
    ...
    public function scoreGoods()
    {
        ...
        $img = M('goods_gallerys')->where(array(
            'goodsId' => $id,
            'shopId' => 0
        ))->select();
        $this->img = $img;
    ...
    public function scorePay()
    {
        ...
        if ($addr) {
            $db = M('areas');
            $addr['province'] = $db->where(array(
                'areaId' => $addr['areaId1']
            ))->getField('areaName');
            $addr['city'] = $db->where(array(
                'areaId' => $addr['areaId2']
            ))->getField('areaName');
            $addr['area'] = $db->where(array(
                'areaId' => $addr['areaId3']
            ))->getField('areaName');
        }
        
        $exisPwd = M('users')->where(array(
            'userId' => session('oto_userId')
        ))->getField('payPwd');
        if ($exisPwd) {
            $exisPwd = 1;
    ...
    public function scorePayHandle()
    {
        if (IS_AJAX) {
            ...
            $goodsInfo = M('integral')->where(array(
                'goodsId' => $id,
                'goodsStock' => array(
                    'gt',
                    0
            ...
            if ($orderId > 0 && $orderSrcNo > 0) {
                ...
                // 操作库存
                $stock = M('integral')->where(array(
                    'goodsId' => $goodsid
                ))->setDec('goodsStock', 1);
                // 扣掉积分
                $userScore = M('users')->where(array(
                    'userId' => session('oto_userId')
                ))->setDec('userScore', $goodsInfo['shopPrice']);
                // 积分变动记录
