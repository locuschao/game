...
class GoodsModel extends BaseModel {
	...
	public function queryOnSaleByPage(){
        ...
        $sql="select g.isSale,g.upTime,g.goodsId,g.saleCount,g.goodsSn,g.goodsName,g.goodsThums,s.shopId,g.marketPrice,g.shopPrice,
		...
		foreach ($info['root'] as $k=>$v){
		    $version=M('goods_versions  as gv')->where(array('gv.goodsId'=>$v['goodsId']))->join('oto_versions as v on gv.versionsId=v.id')->field('v.vName,gv.attrPrice')->order('gv.attrPrice ASC')->select();
	...
	public function queryPenddingByPage(){
	   $shopId=(int)session('WST_USER.shopId');
		...
		$info= $this->pageQuery($sql);
			foreach ($info['root'] as $k=>$v){
		    $version=M('goods_versions  as gv')->where(array('gv.goodsId'=>$v['goodsId']))->join('left join oto_versions as v on gv.versionsId=v.id')->field('v.vName,gv.attrPrice')->order('gv.attrPrice ASC')->select();
		    $info['root'][$k]['lowPrice']=$version[0]['attrPrice'];
		    //最低折扣
		    $info['root'][$k]['zhekou']=sprintf('%0.1f',$version[0]['attrPrice']/$v['shopPrice']*10);
		        $vName='';
	...
	public function addGoods(){
	    ...
	    $info=I('info');
	    $info=trim($info,';');
	    if(!$gameId||!$versionsId){return array('status'=>-1);}
	    $gameInfo=M('game')->where(array('id'=>$gameId))->find();
	    $versionInfo=M('versions')->where(array('id'=>$versionsId))->find();
	  
	    if(empty($gameInfo)||empty($versionInfo)){return array('status'=>-1);}
	    ...
	    foreach ($allItem as $k=>$v){
	       ...
	       if(!$rs){
	           ...
	           $data["goodsName"] = $gameInfo['gameName'];
			...
			if(false !== $rs){
			    ...
			    foreach ($arr as $k=>$v){
			        ...
			        if($temp[0]&&$temp[2]>0){
			            ...
			            $vData['attrStock']=0;
			            $vData['isRecomm']=0;
			            $isExists=M('goods_versions')->where(array('id'=>$temp[1]))->find();
			            if($isExists){
			                M('goods_versions')->where(array('id'=>$temp[1]))->save($vData);
			            }else{
			                M('goods_versions')->add($vData);
			            }
			        }
	 ...
	 */
	 public function get(){
		...
		if(empty($goods))return array();
	    $versions=M('goods_versions')->where(array('shopId'=>$shopId,'goodsId'=>$id))->select();
	 ...
	 public function getGameVersions($id){
	     ...
	     $info=M('game_versions as gv')->join('oto_versions as v on v.id=gv.vid')->where(array('gv.gid'=>$id))->field('v.*')->order('v.id ASC')->select();
	     return $info;
	 }
        ...
        if($this->checkEmpty($data,true)){
            ...
            if(false !== $rs){
                ...
                if($temp){
                    ...
                    $a->add($Auction);
                    $mg=M('Goods');
                    $mg->where('goodsId='.$goodsId.' and shopId='.$shopId)->limit(1)->setDec('goodsStock',$Auction["auctionWinNum"]);
                }
		...
		if($isBook==1){
	 	...
	 	if($isGroup){
			...
			if($goods[0]['attrCatId']>0){
				...
				$priceAttrs = $this->query($sql);
				if(!empty($priceAttrs))$goods[0]['goodsStock'] = $priceAttrs[0]['skAttrStock'];
			}
		}else{
		 	if($goods[0]['attrCatId']>0){
