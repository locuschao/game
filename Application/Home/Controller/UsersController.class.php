...
class UsersController extends BaseController
{
    ...
    public function toEditPass()
    {
        ...
        $USER = session('WST_USER');
        $usersModel = M('Users');
        $payPwd = $usersModel->where('userId = ' . $USER['userId'])->getField('payPwd');
        if (! is_null($payPwd)) {
            $this->assign('payPwd', '1');
        }
     ...
     * 修改用户支付密码
     */
    public function editPassPay()
    {
        $this->isAjaxLogin();
        $USER = session('WST_USER');
        $m = D('Home/Users');
        $rs = $m->editPassPay($USER['userId']);
        $this->ajaxReturn($rs);
    }
    ...
    public function checkPhoneVerify()
    {
        ...
        if (session('findPass.phoneVerify') == $phoneVerify) {
            ...
            if (! empty($user)) {
                ...
                $rs['url'] = "http://" . $_SERVER['HTTP_HOST'] . U('Home/Users/toResetPass', array(
                    'key' => $key
                ));
            }
        }
    ...
    public function getEmailVerify()
    {
        ...
        $url = "http://" . $_SERVER['HTTP_HOST'] . U('Home/Users/toResetPass', array(
            'key' => $key
        ));
