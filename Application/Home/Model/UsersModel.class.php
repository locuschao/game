...
class UsersModel extends BaseModel
{
    ...
    public function getUserInfo($loginName, $loginPwd)
    {
        ...
        $rs = $m->where(" loginName ='%s' AND loginPwd ='%s' ", array(
            $loginName,
            $loginPwd
        ))->find();
        return $rs;
     ...
     * 修改用户支付密码
     */
    public function editPassPay($id)
    {
        $rd = array(
            'status' => - 1
        );
        ...
        $m = M('users');
        $rs = $m->where('userId=' . $id)->find();
        if (is_null($rs['payPwd'])) {
            $data["payPwd"] = I("passPay");
            if ($this->checkEmpty($data, true)) {
                $data["payPwd"] = md5(I("passPay") . $rs['loginSecret']);
                $rs = $m->where("userId=" . $id)->save($data);
                if (false !== $rs) {
                    $rd['status'] = 1;
                }
            ...
            $data["payPwd"] = I("newPassPay");
            if ($this->checkEmpty($data, true)) {
                // 核对密码
                if ($rs['payPwd'] == md5(I("oldPassPay") . $rs['loginSecret'])) {
                    $data["payPwd"] = md5(I("newPassPay") . $rs['loginSecret']);
                    $rs = $m->where("userId=" . $id)->save($data);
                    if (false !== $rs) {
                        $rd['status'] = 1;
                    }
