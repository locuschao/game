...
class AttributesModel extends BaseModel
{
    ...
    public function queryByPage()
    {
        ...
        $m = M('attributes');
        $shopId = (int) session('WST_USER.shopId');
        return $m->where('shopId=' . $shopId . ' and attrFlag=1 and catId=' . $catId)
            ->order('attrSort asc,attrId asc')
            ->select();
    }
    ...
    public function queryByListForGoods()
    {
        ...
        $m = M('attributes');
        $shopId = (int) session('WST_USER.shopId');
        $rs = $m->where('shopId=' . $shopId . ' and attrFlag=1 and catId=' . $catId)
            ->order('attrSort asc,attrId asc')
            ->select();
        foreach ($rs as $key => $v) {
            // 分解下拉和多选的选项
