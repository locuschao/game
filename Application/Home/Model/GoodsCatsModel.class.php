...
class GoodsCatsModel extends BaseModel
{
    ...
    public function getGoodsCatsAndGoodsForIndex($areaId2)
    {
        ...
        // S("WST_CACHE_GOODS_CAT_GOODS_WEB_".$areaId2,0);
        if (! $cats) {
            ...
            for ($i = 0; $i < count($rs1); $i ++) {
                ...
                for ($j = 0; $j < count($rs2); $j ++) {
                     ...
                     */
                    // 商品列表不需要显示属性
                    $sql = "SELECT sp.shopName, g.saleCount , sp.shopId , g.goodsId , g.goodsName,g.goodsImg, g.goodsThums,g.shopPrice, g.marketPrice,g.goodsSn
							FROM __PREFIX__goods g, __PREFIX__shops sp
							WHERE g.shopId = sp.shopId AND sp.shopStatus = 1 AND g.goodsFlag = 1 AND g.isSale = 1 AND g.goodsStatus = 1 AND g.goodsCatId2 = $cat2Id AND sp.areaId2=$areaId2
							ORDER BY g.saleCount desc limit 8";
                    $grs = $this->query($sql);
                    foreach ($grs as $gkey => $v) {
                        if (intval($v['goodsAttrId']) > 0)
                            $grs[$gkey]['shopPrice'] = $v['attrPrice'];
                    }
                    $rs2[$j]["goods"] = $grs;
                    $cats2[] = $rs2[$j];
                }
                ...
                // 查询二级分类下的商品
                $sql = "SELECT sp.shopName, g.saleCount , sp.shopId , g.goodsId , g.goodsName,g.goodsImg, g.goodsThums,g.shopPrice,g.marketPrice, g.goodsSn,ga.id goodsAttrId,ga.attrPrice
						FROM __PREFIX__goods g left join __PREFIX__goods_attributes ga on g.goodsId=ga.goodsId and ga.isRecomm=1, __PREFIX__shops sp
                ...
                foreach ($jgrs as $gkey => $v) {
                    if (intval($v['goodsAttrId']) > 0)
                        $jgrs[$gkey]['shopPrice'] = $v['attrPrice'];
                }
                $rs1[$i]["jpgoods"] = $jgrs;
