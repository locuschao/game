...
class JoinController extends BaseController {
    ...
    public function protocol(){
        $isExis = M ( 'shops' )->where ( array ( 'userId' => session('oto_userId') ) )->find ();
    ...
    public function myShop(){
        ...
        $map['shopStatus']=1;
        $map['shopFlag']=1;
        $this->shopInfo=M('shops')->where($map)->field('shopName,shopSn')->find();
        // 本月订单
        $month['isPay']=1;
    ...
    public function sysNotice(){
        ...
        $this->info=M('messages')->where($map)->order('id DESC')->select();
        $this->display();
    }
    ...
    public function toJoin(){
        ...
        $this->sort=M('goodsCats')->where(array('catFlag'=>1,'isShow'=>1,'parentId'=>0))->field('catId,catName')->select();
        $this->display();
    }
    
    //提示
    public function tip(){
        $isExis = M ( 'shops' )->where ( array ( 'userId' => session('oto_userId') ) )->getField('shopStatus');
        $info='';
        switch ($isExis){
            case 0:$info="请耐心等待审核";break;
            case 1:$info="你已经是商家";break;
            case -1:$info="你的申请已被拒绝";break;
            case -2:$info="店铺停止营业";break;
        }
        $this->info=$info;
        $this->display();
    }
    ...
    public function toJoinHandle() {
        ...
        $isExis = M ( 'shops' )->where ( array ( 'userId' => $data ['userId'] ) )->find ();
        if ($isExis) {
            //已经申请过
