...
class CouponController extends BaseController {
    ...
    public function getCouponHandel(){
        ...
        if(IS_AJAX){
            $id=I('id');
            $uinfo=M('youhui_user_link')->where(array('youhui_id'=>$id))->find();
    ...
    public function coupon(){
        ...
        $notUse=M('youhui_user_link')->field($field)->where(array('u.user_id'=>session('oto_userId')))->join("as u  join oto_youhui as y on u.youhui_id=y.id and  $time between begin_time and end_time left join oto_shops as s on s.shopId=y.supplier_id" )->select();
        ...
        //过期没使用
        $field="y.deal_cate_id,u.surplus,s.shopName,y.deal_cate_id,y.good_id,y.supplier_id,y.shop_cat_id,y.brand_id,y.name,y.youhui_scope,y.good_id,y.supplier_id,y.begin_time,y.end_time,y.youhui_type,y.total_fee,y.breaks_menoy,y.deal_cate_id,y.expire_day,y.city_id";
        $expireNoUse=M('youhui_user_link')->field($field)->join("as u  join oto_youhui as y on u.youhui_id=y.id and  $time not between begin_time and end_time left join oto_shops as s on s.shopId=y.supplier_id where y.id not in(select youhui_id from oto_youhui_use_record where userId=50) and u.user_id= $uid" )->select();
