...
class GroupController extends Controller{
    ...
    public function search(){
        ...
        switch($sorts){
            //智能排序
            case 'smart':$order=" order by gg.endTime ASC ";break;
            //距离最近
            case 'lately':$order=" order by gg.endTime ASC ";break;
            //起送价最低
            case 'freight':$order=" order by ss.deliveryStartMoney ASC ";break;
            // 销量最高
            case 'volume':$order="order by monthSale DESC ";break;
        }
        $where="";
        if($brand){
...
Catesql;
        ...
        $ids3=trim($ids,',');
        //店铺分类
        $cat3=M('goods_cats')->where(array('isShow'=>1,'catFlag'=>1,'catId'=>array('in',$ids3)))->order('catSort ASC')->select();
        //二级分类
        $ids2='';
        foreach ($cat3 as $k=>$v){
            $ids2.=$v['parentId'].',';
        ...
        $ids2=implode(',', $ids2);
        $cat2=M('goods_cats')->where(array('isShow'=>1,'catFlag'=>1,'catId'=>array('in',$ids2)))->order('catSort ASC')->select();
        //一级分类 
        $ids1='';
        foreach ($cat2 as $k=>$v){
            $ids1.=$v['parentId'].',';
        ...
        $ids1=implode(',', $ids1);
        $cat1=M('goods_cats')->where(array('isShow'=>1,'catFlag'=>1,'catId'=>array('in',$ids1)))->order('catSort ASC')->select();
