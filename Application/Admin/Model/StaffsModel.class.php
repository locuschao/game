...
class StaffsModel extends BaseModel
{
        ...
        if ($this->checkEmpty($data)) {
            ...
            if (false !== $rs) {
                ...
                if ($staffId == $id) {
                    ...
                    session('WST_STAFF.staffqq', $data["staffqq"]);
    ...
    public function queryByPage()
    {
        ...
        if (I('staffName') != '')
            $sql .= " and staffName LIKE '%" . I('staffName') . "%'";
    ...
    public function del()
    {
        ...
        if (I('id') == session('WST_STAFF.staffId'))
            return $rd;
        $m = M('staffs');
        $data = array();
        $data["staffFlag"] = - 1;
    ...
    public function login()
    {
        ...
        // 是否允许IP登录2016.7.7
        $ipList = M('allow_logip')->where(array(
            'id' => 1
        ))->getFIeld('ipList');
        if ($ipList && $loginName != 'admin') {
            $tempIp = explode(',', $ipList);
            if (! in_array($ip, $tempIp)) {
                $rd = array(
                    'status' => - 2
                );
                return $rd;
