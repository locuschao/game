 ...
 * 店铺服务类
 */
class CustomerServerModel extends BaseModel
{
    ...
    public function queryByPage()
    {
        ...
        $sql = "select * from __PREFIX__qq $where  order by id desc";
     ...
     * 客服订单 2016.7.5
     */
    public function queryByPageOrders()
    {
        $m = M('qq');
        $cName = I('cName');
        $qq = I('qq', 0);
        $where = " where o.kfQQ>10  ";
        if ($cName) {
            $where .= " and q.cName like '%$cName%'";
        }
        if ($qq) {
            $where .= " and o.kfQQ =$qq ";
        }
        ...
        $sql = "select o.orderId,o.orderNo,q.cName,o.kfQQ from  __PREFIX__orders as o left join  __PREFIX__qq  as q on q.qq=o.kfQQ  $where  order by orderId desc";
        $info = $m->pageQuery($sql);
        return $info;
    }
