...
class CommentController extends BaseController
{
    ...
    public function evaluate()
    {
        ...
        $serviceInfo = M('goods_appraises as a')->field($field)
            ->where($map)
            ->join('left join oto_order_goods as g on a.orderId=g.orderId and a.goodsId=g.goodsId left join oto_shops as s on s.shopId=a.shopId')
            ->select();
    ...
    public function serviceEvaluateHandle()
    {
        ...
        $oid = I('oid');
        $gid = I('gid');
        $isPJ = M('goods_appraises')->where(array(
            'orderId' => $oid,
            'goodsId' => $gid
        ))->getField('serviceScore');
        if ($isPJ) {
            $this->ajaxReturn(array(
                'status' => - 2
            ));
        ...
        $res = M('goods_appraises')->where(array(
            'orderId' => $oid,
            'goodsId' => $gid,
    ...
    public function addEvaluate()
    {
        ...
        $this->goodsInfo = M('order_goods')->where(array(
            ...
            ->field('goodsThums,goodsName,goodsAttrName')
            ->find();
        $this->display();
    }
    // 评价处理
    public function addEvaluateHandle()
    {
        ...
        $isExisA = $DB->where(array(
            'orderId' => $orderId,
            'goodsId' => $goodsId
        ))->find();
        if ($img) {
            $sDB = M('share');
            $isExisB = $sDB->where(array(
                'goodsId' => $goodsId,
                'orderId' => $orderId
            ))->find();
            if ($isExisB) {
    ...
    public function addToEvaluate()
    {
        ...
        $this->goodsInfo = M('order_goods')->where(array(
            ...
            ->field('goodsThums,goodsName,goodsAttrName')
            ->find();
        $this->comment = M('goods_appraises')->where(array(
            'orderId' => $oid,
            'goodsId' => $gid
        ))->find();
        $this->display();
    ...
    public function addToEvaluateHandle()
    {
        ...
        if ($img) {
            ...
            if ($B) {
                ...
                M('goods_appraises')->where(array(
                    'orderId' => $orderId,
                    'goodsId' => $goodsId
                ))->setField('anonymity', $anonymity);
            } else {
                $this->ajaxReturn(array(
                    'status' => - 1
                ));
