...
class ServiceController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->list = M('article_cats')->field('catId,catName')
            ->where(array(
            'catFlag' => 1,
            'parentId' => 0
        ))
    ...
    {
        $cid = I('get.cid'); // 分类ID
        $this->list = M('articles')->field('articleId,articleTitle')
            ->where(array(
            'isShow' => 1,
            'catId' => $cid
        ))
    ...
    {
        $id = I('get.id');
        $info = M('articles')->where(array(
            'articleId' => $id
        ))
            ->field('articleTitle,articleContent')
            ->find();
