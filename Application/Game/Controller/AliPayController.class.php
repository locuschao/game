        ...
        if ($verify_result) {
            ...
            if ($_POST['trade_status'] == 'TRADE_FINISHED') {
                ...
                if ($orderType == 1) {
                     ...
                     * @author peng
                     * @date 2017-04
                     * @descreption 验证实际支付的钱是否能订单的一致
                     */
                    if($ispayInfo['needPay'] != $payMoney) {
                        logResult($log_name, "【实际支付跟订单金额不一致】:\n" . $orderids . "\n");
                        return;
                    }    
                    ...
                    if ($res && $E && $D) {
                         ...
                         * @description  添加平台流水
                         */
                        M('platfrom_account')->add(array(
                            'orderId' =>$orderids,
                            'time'     =>time(),
                            'income'   => $ispayInfo['needPay'],
                            'remark'   => '买家购物支付成功',
                            'orderNo'  => $ispayInfo['orderNo']
                        ));
                        M('data_tmp')->where('id=1')->setInc('value',$ispayInfo['needPay']);  //魏永就  改变平台暂时金额
                        /**
                        * @author peng
                        * @date 2017-01
                        * @descreption 代金券的发货
                        */
                        if($ispayInfo['shopId']==0) D('Game/Voucher')->voucherFahuo($orderids);
                        /**
                        * @author peng
                        * @date 2017-01
                        * @descreption 支付完成自动发货
                        */
                        D('Home/SdkAgent')->autoFahuo($orderids);
     ...
     */
     public function returnurl2(){
         if ($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
             ...
             * @author peng
             * @date 2017-02
             * @descreption 根据类型来决定返回的地址
             */
            //判断订单类型，1为订单，2为充值
            $order_pay_info = M('orders_payid')->where(array(
                'pid' => $_GET['out_trade_no'] //商户订单号
            ))->find();
            if($order_pay_info['type']==2) {
                $call_back_uri=U('/Game/Ucenter/wallet');
            }else if($order_pay_info['type']==1) {
                $call_back_uri=U('/Game/Orders/orderDetail',['id'=>$order_pay_info['orderId']]);
            }
            
            header('location:'.$call_back_uri);//跳转到配置项中配置的支付成功页面；
