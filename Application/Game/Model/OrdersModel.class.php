...
class OrdersModel extends BaseModel
{
    ...
    public function addOrders($userId, $consigneeId, $payway, $needreceipt, $orderunique, $isself)
    {
        ...
        $orderNo = $this->getOrderSn($orderSrcNo);
        ...
        if ($attrId) {
            ...
            #加上会员价格的因素
            $needPay = $num * $this->getNeedPay($attrInfo);
        ...
        $data["selfBuildAccount"] = I('selfBuildAccount',''); 
        ...
        if($userInfo['rank']==0 && $userInfo['partnerId']>0 && $goodsInfo['shopId']==0){
            $data['is_fencheng']=1;
        }else{
            $data['is_fencheng']=0;
        }*/
     ...
     * @descreption 生成订单编号
     */
     public function getOrderSn($orderSrcNo){
        return date('Y') . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_RIGHT).$orderSrcNo; 
     }
     ...
     public function getNeedPay($attrInfo){
        ...
        $attrInfo['low_member_price']=(float)$attrInfo['low_member_price'];
        $attrInfo['mid_member_price']=(float)$attrInfo['mid_member_price'];
        $attrInfo['heigh_member_price']=(float)$attrInfo['heigh_member_price'];
        
        $rank=M('users')->find(session('oto_userId'))['rank'];
        if($rank==3){
            $siggle_price=$attrInfo['low_member_price']?:$attrInfo['attrPrice'];
        }else if($rank==2){
            $siggle_price=($attrInfo['mid_member_price']?:$attrInfo['low_member_price'])?:$attrInfo['attrPrice'];
        }else if($rank==1){
            
            $siggle_price=($attrInfo['heigh_member_price']?:($attrInfo['mid_member_price']?:$attrInfo['low_member_price']))?:$attrInfo['attrPrice'];
            
        }else{
            $siggle_price=$attrInfo['attrPrice'];
        }
        return $siggle_price;
     }
