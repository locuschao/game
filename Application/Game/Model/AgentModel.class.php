...
class AgentModel extends Model
{
    ...
    public function fansMoreInfo($levelFans)
    {
        ...
        foreach ($levelFans as $key => $value) {
             ...
             * @author peng	
             * @date 2017-01-10
             * @descreption 分成改成了oto_percentage_log
             */
            $sql = "select sum(gain_price) as price,count(*) as count from __PREFIX__percentage_log where `userId`='{$value['userId']}'";
            $order = M()->query($sql);
    ...
    public function thisFansOrder($cid)
    {
      ...
      /*   $data = M('distribution_log')->where(array(
        ...
        ->join('left join oto_orders as o on o.orderId=d.orderId')
        ->where(array(
            'uid' => $userId,
            'cid' => $cid,
            'o.orderNo' =>array('gt',0)
