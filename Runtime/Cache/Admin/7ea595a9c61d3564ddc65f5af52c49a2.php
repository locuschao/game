...
<script>
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该游戏版本吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   ...
		   $.post("<?php echo U('Admin/Game/delGameType');?>",{id:id},function(data,textStatus){
					...
					if(json.status=='1'){
                        ...
                        Plugins.closeWindow();
                        parent.showMsg({msg:'该游戏类型绑定了游戏!',status:'danger'});
                    }else{
						Plugins.closeWindow();
...
<body class='wst-page'>
	<form method='POST' action="<?php echo U('Admin/Game/listType');?>">
		<div class='wst-tbar'>
			类型名称：<input type='text' id='gameName' name='gameName'
			...
			<a class="btn btn-success glyphicon glyphicon-plus"
				href="<?php echo U('Admin/Game/addGameType');?>" style='float: right'>新增</a>
			<?php } ?>
		</div>
	</form>
	<div class='wst-body'>
			...
			<tbody>
				<?php if(is_array($gameType)): $i = 0; $__LIST__ = $gameType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php if(in_array('dplb_02',$WST_STAFF['grant'])){ ?> <a
						...
						href="<?php echo U('Admin/Game/addGameType',array('id'=>$vo['typeId'],'src'=>'index'));?>">修改</a>&nbsp;
						<?php } ?> <?php if(in_array('dplb_03',$WST_STAFF['grant'])){ ?>
						<button type="button"
							class="btn btn-default glyphicon glyphicon-trash"
							onclick="javascript:del(<?php echo ($vo['typeId']); ?>)">
							刪除
							</buttona>
							<?php } ?></td>
