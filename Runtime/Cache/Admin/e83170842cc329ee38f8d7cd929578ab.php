...
<script>
   function handle(id){
	   Plugins.confirm({title:'信息提示',content:'您确定此提现已完成吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   ...
		   $.post("<?php echo U('Admin/Tixian/bizTixianHandle');?>",{id:id},function(data,textStatus){
...
<body class='wst-page'>
	<form method='POST' action="<?php echo U('Admin/Tixian/bizTixianList');?>">
		<div class='wst-tbar'>
			商家名称<input type='text' id='shopName' name='shopName'
				class='form-control wst-ipt-10' value='<?php echo ($shopName); ?>' /> 提现ID<input
	...
	<!--二次开发 --start-->
	<div class="dataExp">
		<form action="/index.php/Admin/Tixian/bizExp" method="post" id="timeForm">
			<table>
				<tr>
					<td>
						提现时间：
					...
					<td><button type="submit" class="btn btn-primary">导出商家提现数据</button></td>
		...
		<table class="table table-hover table-striped table-bordered wst-list">
			...
			<tbody>
				<?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php if(in_array('sjtx_01',$WST_STAFF['grant'])){ ?> <?php if($vo['txStatus'] == 0): ?><button type="button" class="btn btn-default "
