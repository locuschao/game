...
/**
 * @param $var  要输出的变量
 * @param bool $flag  如果为true,则输出的变量写进文件里
 * @param string $fileName  数量输出的文件名
 * @author 魏永就
 */
function dd($var,$flag=false,$fileName='testXml.txt')
{
    if(!APP_DEBUG)
        return ;
    $arr=debug_backtrace();
    if(!$flag)
    {
        ...
        if($arr)
        {
            echo "file:  ".$arr[0]['file']."<br>";
            echo "line:  ".$arr[0]['line']."<br>";
        }
        echo "</div>";
 ...
 * @param string $fileName  数量输出的文件名
 * @author 魏永就
 * @function de 和dd 不同处：de输出信息后，程序就会终止
 */
function de($var,$flag=false,$fileName='testXml.txt')
{
    if(!APP_DEBUG)
        return ;
    $arr=debug_backtrace();
